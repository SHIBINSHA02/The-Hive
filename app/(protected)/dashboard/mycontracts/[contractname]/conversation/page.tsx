  ...
  if (error && !conversation)
    return (
      <div className="p-6 text-red-600">
        ...
        <p>{error}</p>
        <Link
          href={`/dashboard/mycontracts/${encodeURIComponent(contractId)}`}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {selectedThread.messages?.length ? (
                  selectedThread.messages.map((msg, idx) => (
                    <div
                      key={msg._id ?? `msg-${idx}`}
                      className={`flex ${msg.senderRole === "system" ? "justify-center" : msg.senderRole === "client" ? "justify-start" : "justify-end"}`}
                    >
                      <div
